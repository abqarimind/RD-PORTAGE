...
type Chip = { small: string } & ({ text: string } | { num: number; fmt: (n: number) => string });
        ...
        {chips.map((c, i) => (
          <Reveal key={c.small} delayMs={i * 70}>
            <SpotCard className="h-full rounded-2xl border border-[#ECEEF3] bg-white px-4 py-4 text-center transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-md md:text-left">
              ...
              <p className="mt-1 text-xs font-medium text-[#7A8093]">{c.small}</p>
            </SpotCard>
          </Reveal>
        ))}
      </div>
    ...
    <section id="methode" className="border-y border-[#ECEEF3] bg-[#FAFBFD]">
      <div className="mx-auto max-w-page px-4 py-14">
        <Reveal>
          ...
          <h2 className="mt-2 max-w-xl text-3xl font-extrabold leading-tight tracking-tight md:text-4xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {steps.map(([num, title, body, tint], i) => (
            <Reveal key={num} delayMs={i * 90} className="h-full">
              <SpotCard
                className="relative h-full overflow-hidden rounded-3xl p-6 transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-xl"
                style={{ backgroundColor: tint }}
              >
                <p className="text-5xl font-extrabold tabular-nums" style={{ color: BRASS, opacity: 0.25 }}>
                  {num}
                </p>
    ...
    <section id="tarif" className="mx-auto max-w-page px-4 py-16 text-center">
      <Reveal>
        <SpotCard className="rounded-3xl px-6 py-12 md:py-16" style={{ backgroundColor: PEACH }}>
          ...
          <p className="mt-3 text-5xl font-extrabold tabular-nums" style={{ color: BRASS }}>
            <CountUp value={4} format={(n) => `${Math.round(n)} %`} />
          </p>
...
function Styles() {
  return (
    <style jsx global>{`
      ...
      /* magic-card: cursor-following brass border + soft inner glow */
      .spotcard {
        position: relative;
        isolation: isolate;
      }
      .spotcard::before {
        ...
        background: radial-gradient(170px circle at var(--mx, 50%) var(--my, 50%), rgba(176, 141, 87, 0.55), transparent 62%);
      ...
      .spotcard::after {
        ...
        opacity: 0;
        transition: opacity 0.3s ease;
        background: radial-gradient(230px circle at var(--mx, 50%) var(--my, 50%), rgba(176, 141, 87, 0.1), transparent 60%);
      }
